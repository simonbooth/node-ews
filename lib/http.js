...
module.exports = {
  request: function(rurl, data, callback, exheaders, exoptions) {
    ...
    if (options.username.indexOf('\\') > -1) {
      options.domain = options.username.split('\\')[0].toUpperCase()
      options.username = options.username.split('\\')[1].toLowerCase()
    }
  ...
  request2: function (rurl, data, callback, exheaders, exoptions) {
    // parse args
    var args = Array.prototype.slice.call(arguments)
    rurl = args.shift()
    data = typeof args[0] === 'string' ? args.shift() : null
    ...
    ntlmClient.request(ntlmClientOptions)
      .then(function (res) {
        var body = res.body
        if (typeof body === 'string') {
          // Remove any extra characters that appear before or after the SOAP
